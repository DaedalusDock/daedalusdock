...
/proc/jps_path_to(atom/movable/invoker, atom/end, max_distance = 30, mintargetdist, list/access, simulated_only = TRUE, turf/exclude, skip_first=TRUE, diagonal_handling=DIAGONAL_REMOVE_CLUNKY)
	...
	if(!SSpathfinder.jps_pathfind(invoker, end, max_distance, mintargetdist, access, simulated_only, exclude, skip_first, diagonal_handling, await))
		return list()
...
/proc/astar_path_to(
	...
	// We're guarenteed that list will be the first list in pathfinding_finished's argset because of how callback handles the arguments list
	var/datum/callback/await = CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(__pathfinding_finished), packet)
	if(!SSpathfinder.astar_pathfind(invoker, end, max_distance, mintargetdist, access, simulated_only, exclude, skip_first, use_diagonals, await, heuristic))
		return list()
