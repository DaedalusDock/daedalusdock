...
/datum/controller/subsystem/hardspace/Initialize(start_timeofday)
	...
	steptypes = list()
	message_admins(json_encode(subtypesof(/datum/salvage_step) || "FUCK THERE'S NO SUBTYPES??"))
	for(var/datum/salvage_step/steptype as anything in subtypesof(/datum/salvage_step))
