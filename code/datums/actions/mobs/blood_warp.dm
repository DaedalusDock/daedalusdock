...
/datum/action/cooldown/mob_cooldown/blood_warp/proc/blood_warp(atom/target)
	...
	if(found_bloodpool)
		...
		playsound(get_turf(owner), 'sound/magic/enter_blood.ogg', 100, TRUE, -1, ignore_walls = TRUE)
		owner.forceMove(get_turf(found_bloodpool))
		playsound(get_turf(owner), 'sound/magic/exit_blood.ogg', 100, TRUE, -1, ignore_walls = TRUE)
