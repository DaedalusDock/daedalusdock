/datum/component/personal_crafting/Initialize()
	if(ismob(parent))
		RegisterSignal(parent, COMSIG_MOB_CLIENT_LOGIN, PROC_REF(create_mob_button))on))ut)ton))
...
/datum/component/personal_crafting/proc/create_mob_button(mob/user, client/CL)
	...
	CL.screen += C
	RegisterSignal(C, COMSIG_CLICK, PROC_REF(component_ui_interact))ct))er)act))
...
/datum/component/personal_crafting/proc/component_ui_interact(atom/movable/screen/craft/image, location, control, params, user)
	...
	if(user == parent)
		INVOKE_ASYNC(src, PROC_REF(ui_interact),ct),ct),ct), user)
