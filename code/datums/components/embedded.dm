...
/datum/component/embedded/Initialize(obj/item/I,
	...
	if(damage > 0)
		...
		limb.receive_damage(brute=(1-pain_stam_pct) * damage,blocked=armor, sharpness = I.sharpness)
...
/datum/component/embedded/proc/tweezePluck(obj/item/possible_tweezers, mob/user)
	...
	if(!do_after(user, victim, pluck_time, DO_PUBLIC, display = possible_tweezers))
		if(self_pluck)
