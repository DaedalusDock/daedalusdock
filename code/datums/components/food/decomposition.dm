...
/datum/component/decomposition/proc/handle_movement()
	...
	timerid = addtimer(CALLBACK(src, PROC_REF(decompose))se))se))se)), time_remaining, TIMER_STOPPABLE | TIMER_UNIQUE)
