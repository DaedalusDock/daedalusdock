...
/datum/component/spirit_holding/proc/on_examine(datum/source, mob/user, list/examine_list)
	SIGNAL_HANDLER
	if(!bound_spirit)
		examine_list += span_info("[parent] sleeps. Use [parent] in your hands to attempt to awaken it.")
		return
	examine_list += span_info("[parent] is alive.")
...
/datum/component/spirit_holding/proc/attempt_spirit_awaken(mob/awakener)
	...
	var/mob/dead/ghost/candidates = poll_ghost_candidates("Do you want to play as the spirit of [awakener.real_name]'s blade?", ROLE_PAI, FALSE, 100, POLL_IGNORE_POSSESSED_BLADE)
	...
	var/mob/dead/ghost/chosen_spirit = pick(candidates)
