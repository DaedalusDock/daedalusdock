...
/datum/component/spirit_holding/proc/attempt_exorcism(mob/exorcist)
	...
	playsound(parent, 'sound/hallucinations/veryfar_noise.ogg',40,TRUE, ignore_walls = TRUE)
	if(!do_after(exorcist, exorcised_movable, 4 SECONDS))
		return
	playsound(parent, 'sound/effects/pray_chaplain.ogg',60,TRUE, ignore_walls = TRUE)
