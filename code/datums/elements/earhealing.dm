...
/datum/element/earhealing/Attach(datum/target)
	...
	RegisterSignal(target, list(COMSIG_ITEM_EQUIPPED, COMSIG_ITEM_UNEQUIPPED), PROC_REF(equippedChanged))

/datum/element/earhealing/Detach(datum/target)
	. = ..()
	UnregisterSignal(target, list(COMSIG_ITEM_EQUIPPED, COMSIG_ITEM_UNEQUIPPED))
	user_by_item -= target
