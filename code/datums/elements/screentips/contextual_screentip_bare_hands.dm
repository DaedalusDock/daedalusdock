...
/datum/element/contextual_screentip_bare_hands/proc/on_requesting_context_from_item(
	...
	if (!isnull(lmb_text))
		context[SCREENTIP_CONTEXT_LMB] = living_user.istate.harm ? lmb_text_combat_mode : lmb_text

	if (!isnull(rmb_text))
		context[SCREENTIP_CONTEXT_RMB] = living_user.istate.harm ? rmb_text_combat_mode : rmb_text
