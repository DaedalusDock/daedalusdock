...
/datum/symptom/flesh_eating/sync_properties(list/properties)
	...
	if(!.)
		return
	if(properties[PATHOGEN_PROP_RESISTANCE] >= 7) //extra bleeding
		bleed = TRUE
	if(properties[PATHOGEN_PROP_TRANSMITTABLE] >= 8) //extra stamina damage
		pain = TRUE
...
/datum/symptom/flesh_death/sync_properties(list/properties)
	...
	if(!.)
		return
	if(properties[PATHOGEN_PROP_STEALTH] >= 5)
		suppress_warning = TRUE
	if(properties[PATHOGEN_PROP_STAGE_RATE] >= 7) //bleeding and hunger
		chems = TRUE
