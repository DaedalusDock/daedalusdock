...
/datum/symptom/heal/sync_properties(list/properties)
	...
	if(!.)
		return
	if(properties[PATHOGEN_PROP_STAGE_RATE] >= 6) //stronger healing
		power = 2
...
/datum/symptom/heal/metabolism/sync_properties(list/properties)
	...
	if(!.)
		return
	if(properties[PATHOGEN_PROP_STAGE_RATE] >= 10)
		triple_metabolism = TRUE
	if(properties[PATHOGEN_PROP_STEALTH] >= 3)
		reduced_hunger = TRUE
...
/datum/symptom/heal/coma/sync_properties(list/properties)
	...
	if(properties[PATHOGEN_PROP_STAGE_RATE] >= 7)
		power = 1.5
	if(properties[PATHOGEN_PROP_RESISTANCE] >= 4)
		stabilize = TRUE
...
/datum/symptom/heal/coma/on_stage_change(datum/pathogen/advance/A)  //mostly copy+pasted from the code for self-respiration's TRAIT_NOBREATH stuff
	...
	if(A.stage >= 4 && stabilize)
		ADD_TRAIT(A.affected_mob, TRAIT_NOCRITDAMAGE, PATHOGEN_TRAIT)
	else
		REMOVE_TRAIT(A.affected_mob, TRAIT_NOCRITDAMAGE, PATHOGEN_TRAIT)
	return TRUE
