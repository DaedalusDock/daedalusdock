...
/datum/symptom/mind_restoration/sync_properties(list/properties)
	...
	if(!.)
		return
	if(properties[PATHOGEN_PROP_RESISTANCE] >= 6) //heal brain damage
		trauma_heal_mild = TRUE
	if(properties[PATHOGEN_PROP_RESISTANCE] >= 9) //heal severe traumas
		trauma_heal_severe = TRUE
	if(properties[PATHOGEN_PROP_TRANSMITTABLE] >= 8) //purge alcohol
		purge_alcohol = TRUE
...
/datum/symptom/sensory_restoration/on_process(datum/pathogen/advance/A)
	. = ..()
	if(!.)
		return
