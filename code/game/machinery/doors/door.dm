...
/obj/machinery/door/proc/check_security_level(datum/source, new_level)
	...
	playsound(src, 'sound/machines/boltsup.ogg', 50, TRUE, ignore_walls = TRUE)
...
/obj/machinery/door/play_attack_sound(damage_amount, damage_type = BRUTE, damage_flag = 0)
	switch(damage_type)
		if(BRUTE)
			if(glass)
				playsound(loc, 'sound/effects/glasshit.ogg', 90, TRUE, ignore_walls = TRUE)
			else if(damage_amount)
				playsound(loc, 'sound/weapons/smash.ogg', 50, TRUE, ignore_walls = TRUE)
			else
