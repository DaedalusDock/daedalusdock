...
/obj/machinery/door/Initialize(mapload)
	...
	RegisterSignal(SSsecurity_level, COMSIG_SECURITY_LEVEL_CHANGED, PROC_REF(check_security_level))el))
...
/obj/machinery/door/emp_act(severity)
	...
	if(prob(severity*10 - 20))
		if(secondsElectrified == MACHINE_NOT_ELECTRIFIED)
			...
			addtimer(CALLBACK(src, PROC_REF(unelectrify))fy)), 300)
...
/obj/machinery/door/proc/autoclose_in(wait)
	addtimer(CALLBACK(src, PROC_REF(autoclose))se)), wait, TIMER_UNIQUE | TIMER_NO_HASH_WAIT | TIMER_OVERRIDE)
