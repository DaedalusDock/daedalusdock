...
/obj/machinery/roulette/attackby(obj/item/W, mob/user, params)
	...
	if(istype(W, /obj/item/card/id))
		...
		if(my_card)
			...
			addtimer(CALLBACK(src, PROC_REF(play),(p)lay),ay), user, player_card, chosen_bet_type, chosen_bet_amount, potential_payout), 4) //Animation first
			return TRUE
		else
			...
			if(new_card.registered_account)
				...
				my_card = new_card
				RegisterSignal(my_card, COMSIG_PARENT_QDELETING, PROC_REF(on_my_card_deleted))le)ted))ed))
...
/obj/machinery/roulette/proc/play(mob/user, obj/item/card/id/player_id, bet_type, bet_amount, potential_payout)
	...
	addtimer(CALLBACK(src, PROC_REF(finish_play),ay),ay),ay), player_id, bet_type, bet_amount, payout, rolled_number), 34) //4 deciseconds more so the animation can play
	addtimer(CALLBACK(src, PROC_REF(finish_play_animation))on))on))on)), 30)
...
/obj/machinery/roulette/proc/drop_coin()
	...
	addtimer(CALLBACK(src, PROC_REF(drop_coin))in))in))in)), 3) //Recursion time
...
/obj/item/roulette_wheel_beacon/attack_self()
	...
	used = TRUE
	addtimer(CALLBACK(src, PROC_REF(launch_payload))ad))ad))ad)), 40)
