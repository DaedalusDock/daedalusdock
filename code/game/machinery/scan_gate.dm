...
/obj/machinery/scanner_gate/Initialize(mapload)
	...
	var/static/list/loc_connections = list(
		COMSIG_ATOM_ENTERED = PROC_REF(on_entered),ed),
	)
...
/obj/machinery/scanner_gate/proc/on_entered(datum/source, atom/movable/AM)
	SIGNAL_HANDLER
	INVOKE_ASYNC(src, PROC_REF(auto_scan),an), AM)
...
/obj/machinery/scanner_gate/proc/set_scanline(type, duration)
	...
	add_overlay(type)
	if(duration)
		scanline_timer = addtimer(CALLBACK(src, PROC_REF(set_scanline),ne), "passive"), duration, TIMER_STOPPABLE)
