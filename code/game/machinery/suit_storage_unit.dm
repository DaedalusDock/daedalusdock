...
/obj/machinery/suit_storage_unit/interact(mob/living/user)
	...
	var/choice = show_radial_menu(
		...
		src,
		choices,
		custom_check = CALLBACK(src, PROC_REF(check_interactable),le), user),
...
/obj/machinery/suit_storage_unit/proc/cook()
	...
	if(uv_cycles)
		...
		addtimer(CALLBACK(src, PROC_REF(cook))ok)), 50)
	else
...
/obj/machinery/suit_storage_unit/container_resist_act(mob/living/user)
	...
	if(locked)
		...
		addtimer(CALLBACK(src, PROC_REF(resist_open),en), user), 300)
	else
		open_machine()
		dump_inventory_contents()
