	...
	if(!isnull(next_beep) && (next_beep <= world.time))
		...
		switch(seconds_remaining())
			...
			if(20 to 25)
				...
				volume = 5
		playsound(loc, beepsound, volume, FALSE, ignore_walls = TRUE)
...
/obj/item/bombcore/training
	name = "dummy payload"
	desc = "A Thinktronic replica of a syndicate payload. It's not intended to explode but to announce that it WOULD have exploded, then rewire itself to allow for more training."
	var/defusals = 0
	var/attempts = 0
