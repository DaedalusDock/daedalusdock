...
/obj/machinery/computer/message_monitor/ui_interact(mob/living/user)
	...
	if(hacking || (obj_flags & EMAGGED))
		message = rebootmsg
	var/dat = "<center><span class='internal'>[message]</span></center>"
	...
	switch(screen)
		...
		if(MSG_MON_SCREEN_MAIN)
			...
			if(auth)
				...
				for(var/n = ++i; n <= optioncount; n++)
					dat += "<dd><span class='internal'>&#09;[n]. ---------------</span><br></dd>"
		...
		if(MSG_MON_SCREEN_LOGS)
			...
			for(var/datum/data_pda_message/pda in linkedServer.pda_msgs)
				...
				dat += "<tr><td width = '5%'><center><A href='byond://?src=[REF(src)];delete_logs=[REF(pda)]' style='color: rgb(255,0,0)'>X</a></center></td><td width='15%'>[pda.sender]</td><td width='15%'>[pda.recipient]</td><td width='300px'>[pda.message]</td></tr>"
			dat += "</table>"
		//Hacking screen.
