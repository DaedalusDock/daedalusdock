...
/obj/machinery/telecomms/message_server/Initialize(mapload)
	...
	if (calibrating)
		...
		#warn dummied out PDA logging related code
...
#warn dummied out tcomms pda logging
...
// 		var/datum/data_tablet_msg/msg = new(PDAsignal.format_target(), "[PDAsignal.data["name"]] ([PDAsignal.data["job"]])", PDAsignal.data["message"])
// 		pda_msgs += msg
// 		signal.logged = msg
