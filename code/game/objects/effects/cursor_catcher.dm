...
/atom/movable/screen/fullscreen/cursor_catcher/proc/calculate_params()
	...
	var/our_x = QUESTIONABLE_FLOOR(icon_x / world.icon_size)
	var/our_y = QUESTIONABLE_FLOOR(icon_y / world.icon_size)
	given_turf = locate(owner.x + our_x - QUESTIONABLE_FLOOR(view_list[1]/2), owner.y + our_y - QUESTIONABLE_FLOOR(view_list[2]/2), owner.z)
