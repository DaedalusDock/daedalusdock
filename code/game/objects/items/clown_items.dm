...
/obj/item/soap/afterattack(atom/target, mob/user, proximity)
	...
	if(user.client && ((target in user.client.screen) && !user.is_holding(target)))
		...
		if(do_after(user, target, clean_speedies))
			...
			user.mind?.adjust_experience(/datum/skill/cleaning, max(QUESTIONABLE_FLOOR(cleanies.beauty/CLEAN_SKILL_BEAUTY_ADJUSTMENT),0)) //again, intentional that this does NOT round but mops do.
			qdel(target)
			decreaseUses(user)
		...
		if(do_after(user, target, clean_speedies))
			...
			if(user && isturf(target))
				for(var/obj/effect/decal/cleanable/cleanable_decal in target)
					user.mind?.adjust_experience(/datum/skill/cleaning, QUESTIONABLE_FLOOR(cleanable_decal.beauty / CLEAN_SKILL_BEAUTY_ADJUSTMENT))
