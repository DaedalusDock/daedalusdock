...
/obj/item/his_grace/process(delta_time)
	...
	if(istype(master) && (src in master.held_items))
		switch(bloodthirst)
			if(HIS_GRACE_CONSUME_OWNER to HIS_GRACE_FALL_ASLEEP)
				...
				playsound(master, 'sound/effects/splat.ogg', 100, FALSE, ignore_walls = TRUE)
			else
	...
	if(Adjacent(L))
		if(!L.stat)
			...
			playsound(L, 'sound/weapons/smash.ogg', 50, TRUE, ignore_walls = TRUE)
			playsound(L, 'sound/misc/desecration-01.ogg', 50, TRUE, ignore_walls = TRUE)
...
/obj/item/his_grace/proc/awaken(mob/user) //Good morning, Mr. Grace.
	...
	playsound(user, 'sound/effects/pope_entry.ogg', 100, ignore_walls = TRUE)
	update_appearance()
	move_gracefully()
...
/obj/item/his_grace/proc/consume(mob/living/meal) //Here's your dinner, Mr. Grace.
	...
	playsound(meal, 'sound/misc/desecration-02.ogg', 75, TRUE, ignore_walls = TRUE)
	playsound(src, 'sound/items/eatfood.ogg', 100, TRUE, ignore_walls = TRUE)
	meal.forceMove(src)
...
/obj/item/his_grace/proc/ascend()
	...
	ascended = TRUE
	update_appearance()
	playsound(src, 'sound/effects/his_grace_ascend.ogg', 100, ignore_walls = TRUE)
	if(istype(master))
