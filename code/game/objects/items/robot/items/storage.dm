...
/obj/item/borg/apparatus/Initialize(mapload)
	RegisterSignal(loc.loc, COMSIG_BORG_SAFE_DECONSTRUCT, PROC_REF(safedecon))on))
	return ..()
...
/obj/item/borg/apparatus/pre_attack(atom/atom, mob/living/user, params)
	if(!stored)
		...
		if(itemcheck)
			...
			item.forceMove(src)
			stored = item
			RegisterSignal(stored, COMSIG_ATOM_UPDATED_ICON, PROC_REF(on_stored_updated_icon))on))
			update_appearance()
			return
	else
