...
/obj/item/stack/medical/proc/try_heal(mob/living/patient, mob/user, silent = FALSE)
	...
	if(patient == user)
		...
		if(!do_after(user, patient, self_delay, extra_checks=CALLBACK(patient, TYPE_PROC_REF(/mob/living, try_inject), user, null, INJECT_TRY_SHOW_ERROR_MESSAGE)))

			return
	else if(other_delay)
		if(!silent)
			user.visible_message(span_notice("[user] starts to apply [src] on [patient]."), span_notice("You begin applying [src] on [patient]..."))
		if(!do_after(user, patient, other_delay, extra_checks=CALLBACK(patient, TYPE_PROC_REF(/mob/living, try_inject), user, null, INJECT_TRY_SHOW_ERROR_MESSAGE)))
			return
