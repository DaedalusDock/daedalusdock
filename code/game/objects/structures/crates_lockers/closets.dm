...
/obj/structure/closet/Initialize(mapload)
	if(mapload && !opened) // if closed, any item at the crate's loc is put in the contents
		addtimer(CALLBACK(src, PROC_REF(take_contents),ts), TRUE), 0)
	. = ..()
	update_appearance()
	PopulateContents()
	...
	var/static/list/loc_connections = list(
		COMSIG_CARBON_DISARM_COLLIDE = PROC_REF(locker_carbon),on),
	)
...
/obj/structure/closet/proc/animate_door(closing = FALSE)
	...
	addtimer(CALLBACK(src, PROC_REF(end_door_animation))on)), door_anim_time, TIMER_UNIQUE|TIMER_OVERRIDE|TIMER_CLIENT_TIME)
