...
/obj/structure/fireaxecabinet/disco_flavor(mob/living/carbon/human/user, nearby, is_station_level)
	...
	if(fireaxe)
		if(!open && !broken)
			...
			user.disco_made_easy("axe_lust2", 10, skill_path = /datum/rpg_skill/knuckle_down, failure_text = "Grab it. Grab the axe. You're so close. YOU WANT IT SO BAD.") //they should invent a kind of axe that doesnt talk to you
