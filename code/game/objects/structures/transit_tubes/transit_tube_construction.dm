...
/obj/structure/c_transit_tube/Initialize(mapload)
	. = ..()
	AddComponent(/datum/component/simple_rotation, AfterRotation = CALLBACK(src, PROC_REF(AfterRotation))on)))
...
/obj/structure/c_transit_tube/wrench_act(mob/living/user, obj/item/I)
	...
	if(I.use_tool(src, user, 2 SECONDS, volume=50, extra_checks=CALLBACK(src, PROC_REF(can_wrench_in_loc),oc), user)))
