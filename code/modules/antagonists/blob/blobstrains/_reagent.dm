...
/datum/blobstrain/reagent/attack_living(mob/living/L)
	...
	reagent.expose_mob(L, BLOB_REAGENTATK_VOL, methods = VAPOR, show_message = TRUE, touch_protection = mob_protection)
	send_message(L)

/datum/blobstrain/reagent/blobbernaut_attack(mob/living/L)
	var/mob_protection = L.get_permeability_protection()
	reagent.expose_mob(L, BLOBMOB_BLOBBERNAUT_REAGENTATK_VOL+blobbernaut_reagentatk_bonus, methods = VAPOR, show_message = FALSE, touch_protection = mob_protection)//this will do between 10 and 20 damage(reduced by mob protection), depending on chemical, plus 4 from base brute damage.
...
/datum/reagent/blob/expose_mob(mob/living/exposed_mob, reac_volume, exposed_temperature = T20C, datum/reagents/source, methods=TOUCH, show_message = TRUE, touch_protection = 0, mob/camera/blob/overmind)
