...
/datum/blobstrain/reagent/shifting_fragments/damage_reaction(obj/structure/blob/B, damage, damage_type, damage_flag)
	if((damage_flag == BLUNT || damage_flag == PUNCTURE || damage_flag == LASER || damage_flag == SLASH) && damage > 0 && B.get_integrity() - damage > 0 && prob(60-damage))
...
/datum/reagent/blob/shifting_fragments/expose_mob(mob/living/exposed_mob, reac_volume, exposed_temperature = T20C, datum/reagents/source, methods=TOUCH, show_message = TRUE, touch_protection = 0, mob/camera/blob/overmind)
	. = ..()
