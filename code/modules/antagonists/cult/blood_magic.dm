	...
	if(proximity_flag && IS_CULTIST(user))
		...
		if(istype(target, /obj/item/stack/sheet/iron))
			...
			if(candidate.mmi || candidate.shell)
				...
				playsound(T, 'sound/machines/airlock_alien_prying.ogg', 80, TRUE, ignore_walls = TRUE)
			...
			channeling = TRUE
			playsound(T, 'sound/machines/doors/airlock_open_force.ogg', 50, TRUE, ignore_walls = TRUE)
