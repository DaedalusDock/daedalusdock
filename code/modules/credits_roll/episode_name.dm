	...
	switch(rand(1,100))
		...
		if(41 to 65)
			...
			for(var/client/C in GLOB.clients)
				if(C.key && (C.key == "francinum" || C.key == "kapu1178" || C.key == "gonenoculer5"))
...
/datum/controller/subsystem/credits/proc/draft_episode_names()
	...
	if(istype(ticker.mode, /datum/game_mode/dynamic))
		...
		if(locate(/datum/dynamic_ruleset/midround/blob/from_ghosts/blob) in mode.executed_rules)
