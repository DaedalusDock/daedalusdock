...
/obj/item/kinginyellow/attack_self(mob/user, modifiers)
	...
	if(!(weak_user in cursed))
		...
		if(do_after(user, 8 SECONDS, interaction_key = "kinginyellow"))
		...
		if(do_after(user, 3 SECONDS, interaction_key = "kinginyellow"))
		...
		var/reads = 0
		while(do_after(user, 1 SECONDS, interaction_key = "kinginyellow"))
			reads++
