...
/obj/item/mcobject/messaging/radioscanner/Initialize(mapload)
	...
	RegisterSignal(radio, COMSIG_RADIO_RECEIVE, PROC_REF(incoming_message))
...
/obj/item/mcobject/messaging/radioscanner/proc/set_frequency(datum/mcmessage/input)
	...
	if((isnull(number) || IS_INF_OR_NAN(number)))
		return
...
/obj/item/mcobject/messaging/radioscanner/proc/incoming_message(datum/source, atom/movable/virtualspeaker/speaker, message, freq_num, list/data)
	SIGNAL_HANDLER
