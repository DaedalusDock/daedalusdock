...
/mob/living/carbon/attacked_by(obj/item/I, mob/living/user)
	...
	if(I.stamina_damage)
		apply_damage(I.stamina_damage * (prob(I.stamina_critical_chance) ? I.stamina_critical_modifier : 1), STAMINA, spread_damage = TRUE, wound_bonus = CANT_WOUND, attack_direction = get_dir(user, src))
	if(I.force)
...
/mob/living/carbon/proc/disarm(mob/living/carbon/target)
	...
	if(target.IsKnockdown()) //KICK HIM IN THE NUTS //That is harm intent.
		...
		target.visible_message(
			span_danger("[name] kicks [target.name] in [target.p_their()] chest, knocking the wind out of them!"),
			span_userdanger("You're kicked in the chest by [name]!"),
	...
	if(prob(disarm_success_chance) && length(target.held_items))
		...
		for(var/obj/item/I as anything in target.held_items)
			if(target.dropItemToGround(I))
				target.visible_message(
					span_danger("[target] loses [target.p_their()] grip on \the [I]"),
					span_warning("You drop \the [I]!"),
					null,
					COMBAT_MESSAGE_RANGE
				)
				dropped += I
