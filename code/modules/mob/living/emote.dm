	...
	if(. && ishuman(user))
		...
		if(istype(wings))
			if(wings.wings_open)
				...
				wings.open_wings()
			addtimer(CALLBACK(wings, open ? TYPE_PROC_REF(/obj/item/organ/external/wings/functional, open_wings : /obj/item/organ/external/wings/functionalPROC_REF(close_wings))), wing_time)
