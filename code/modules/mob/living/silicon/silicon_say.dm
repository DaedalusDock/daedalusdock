/mob/living/proc/robot_talk(message)
	...
	var/rendered = "<img class='radio_tag' src='robo.png'>[span_name("[name]")] <span class='message'>[message_a]</span>"
	for(var/mob/M in GLOB.player_list)
		if(M.binarycheck())
			if(isAI(M))
				var/renderedAI = span_binarysay("<img class='radio_tag' src='robo.png'><a href='?src=[REF(M)];track=[html_encode(name)]'>[span_name("[name] ([desig])")]</a> <span class='message'>[message_a]</span>")
