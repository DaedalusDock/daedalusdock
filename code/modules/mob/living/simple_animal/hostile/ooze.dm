...
/mob/living/simple_animal/hostile/ooze/proc/updateNutritionDisplay()
	if(!hud_used) //clientless oozes
		return
	hud_used.screen_objects[HUDKEY_ALIEN_PLASMA_DISPLAY].maptext = MAPTEXT("<div align='center' valign='middle' style='position:relative; top:0px; left:6px'><font color='green'>[round(ooze_nutrition)]</font></div>")
