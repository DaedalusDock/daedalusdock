...
/obj/item/mod/control/examine(mob/user)
	. = ..()
	if(active)
		. += span_info("Charge: [core ? "[get_charge_percent()]%" : "No core"].")
		. += span_info("Selected module: [selected_module || "None"].")
	if(!open && !active)
		. += span_info("You could put it on your <b>back</b> to turn it on.")
		. += span_info("You could open the cover with a <b>screwdriver</b>.")
	else if(open)
		. += span_info("You could close the cover with a <b>screwdriver</b>.")
		. += span_info("You could use <b>modules</b> on it to install them.")
		. += span_info("You could remove modules with a <b>crowbar</b>.")
		. += span_info("You could update the access lock with an <b>ID</b>.")
		. += span_info("You could access the wire panel with a <b>wire tool</b>.")
		if(core)
			. += span_info("You could remove [core] with a <b>wrench</b>.")
		else
			. += span_info("You could use a <b>MOD core</b> on it to install one.")
		if(ai)
			. += span_info("You could remove [ai] with an <b>intellicard</b>.")
		else
			. += span_info("You could install an AI with an <b>intellicard</b>.")
	. += span_info("You could copy/set link frequency with a <b>multitool</b>.")
	. += span_info("<i>You could examine it more thoroughly...</i>")
