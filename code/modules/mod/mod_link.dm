/proc/make_link_visual_generic(datum/mod_link/mod_link, proc_path)
	...
	mod_link.holder.RegisterSignal(user, list(COMSIG_CARBON_APPLY_OVERLAY, COMSIG_CARBON_REMOVE_OVERLAY), proc_path)
	return link_visual
...
/obj/item/mod/control/Hear(message, atom/movable/speaker, message_language, raw_message, radio_freq, list/spans, list/message_mods, message_range, sound_loc)
	...
	if(speaker != wearer && speaker != ai)
		return
	mod_link.visual.say(raw_message, sanitize = FALSE, range = 2)
...
/obj/item/clothing/neck/link_scryer/process(delta_time)
	if(!mod_link.link_call)
		return
	cell.use(min(20 * delta_time, cell.charge))
...
/obj/item/clothing/neck/link_scryer/Hear(message, atom/movable/speaker, message_language, raw_message, radio_freq, list/spans, list/message_mods, message_range, sound_loc)
	...
	if(speaker != loc)
		return
	mod_link.visual.say(raw_message, sanitize = FALSE, range = 3)
