...
/obj/item/mod/module/proc/on_use()
	...
	if(SEND_SIGNAL(src, COMSIG_MODULE_TRIGGERED) & MOD_ABORT_USE)
		return FALSE
	COOLDOWN_START(src, cooldown_timer, cooldown_time)
	addtimer(CALLBACK(mod.wearer, TYPE_PROC_REF(/mob, update_worn_back)), cooldown_time+1) //need to run it a bit after the cooldown starts to avoid conflicts
	mod.wearer.update_worn_back()
...
/obj/item/mod/module/proc/handle_module_icon(mutable_appearance/standing, module_icon_state)
	...
	if(suit_supports_variations_flags && (supports_variations_flags & CLOTHING_DIGITIGRADE_VARIATION))
		icon_to_use = 'modular_pariah/master_files/icons/mob/mod.dmi'
