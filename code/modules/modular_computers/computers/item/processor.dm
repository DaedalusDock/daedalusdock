...
/obj/item/modular_computer/processor/alert_call(datum/computer_file/program/calling_program, alerttext)
	if(!calling_program || !calling_program.alert_able || calling_program.alert_silenced || !alerttext)
		return
	playsound(src, 'sound/machines/twobeep_high.ogg', 50, TRUE)
	machinery_computer.visible_message(span_notice("The [src] displays a [calling_program.filedesc] notification: [alerttext]"))
