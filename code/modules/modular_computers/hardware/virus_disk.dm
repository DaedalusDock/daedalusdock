...
/obj/item/computer_hardware/hard_drive/role/virus/proc/send_virus(target_addr, mob/living/user)
	...
	var/datum/signal/outgoing = new(src, list(SSpackets.pda_exploitable_register = magic_packet))
	...
	switch(user_input_tuple[1])
		...
		if(VIRUS_MODE_GENERIC)
			...
			for(var/iter = 0, iter<itermax, iter++)
				var/reg_name = pick_list(PACKET_NETCLASS, "packet_field_names")
...
///
/// list(VIRUS_MODE_GENERIC) - No input taken, Sets the value of the SSpackets.exploitable_pda_register field on the packet to it's magic_packet.
///
