...
/datum/plant_gene/trait/glow/proc/glow_range(potency)
	return 1.4 + potency * rate

/datum/plant_gene/trait/glow/proc/glow_power(potency)
	return max(potency * (rate + 0.01), 0.1)

/datum/plant_gene/trait/glow/on_new_plant(obj/item/product, newloc)
	. = ..()
	if(!.)
		return
