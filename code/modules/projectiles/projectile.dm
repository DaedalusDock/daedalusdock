...
/obj/projectile/proc/generate_hitscan_tracers(cleanup = TRUE, duration = 3, impacting = TRUE)
	...
	if(muzzle_type && duration > 0)
		...
		thing.color = color
		thing.set_light(l_outer_range = muzzle_flash_range, l_power = muzzle_flash_intensity, l_color = muzzle_flash_color_override || color)
	...
	if(impacting && impact_type && duration > 0)
		...
		thing.transform = matrix
		thing.color = color
		thing.set_light(l_outer_range = impact_light_range, l_power = impact_light_intensity, l_color = impact_light_color_override || color)
		QDEL_IN(thing, duration)
	if(cleanup)
		cleanup_beam_segments()
