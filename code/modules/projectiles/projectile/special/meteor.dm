...
/obj/projectile/meteor/on_hit(atom/target, blocked = FALSE)
	...
	if(. == BULLET_ACT_HIT && isliving(target))
		...
		playsound(target.loc, 'sound/effects/meteorimpact.ogg', 40, TRUE, ignore_walls = TRUE)
...
/obj/projectile/meteor/Bump(atom/hit_target)
	...
	playsound(src.loc, 'sound/effects/meteorimpact.ogg', 40, TRUE, ignore_walls = TRUE)
