	...
	if(ishuman(exposed_mob))
		if(methods & (PATCH|VAPOR))
			...
			if(MUTCOLORS in exposed_human.dna.species.species_traits) //take current alien color and darken it slightly
				var/newcolor = ""
				var/string = exposed_human.dna.mutant_colors[MUTCOLORS_GENERIC_1]
				...
				if(ReadHSV(newcolor)[3] >= ReadHSV("#7F7F7F")[3])
					exposed_human.dna.mutant_colors[MUTCOLORS_GENERIC_1] = newcolor
...
/datum/reagent/spraytan/overdose_process(mob/living/M, delta_time, times_fired)
	...
	if(ishuman(M))
		...
		if(N.dna.species.use_skintones)
			...
			N.dna.mutant_colors[MUTCOLORS_GENERIC_1] = "#ff8800"
...
/datum/reagent/determination/on_mob_end_metabolize(mob/living/carbon/M)
	if(significant)
		var/stam_crash = 0
		for(var/datum/wound/W as anything in M.get_wounds())
			stam_crash += (W.current_stage + 1) * 3 // spike of 3 stam damage per wound severity (moderate = 6, severe = 9, critical = 12) when the determination wears off if it was a combat rush
...
/datum/reagent/determination/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	...
	var/num_wounds = length(M.get_wounds())
	M.adjustBruteLoss(-0.5 * num_wounds * REM * delta_time)
