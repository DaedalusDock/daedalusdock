...
/datum/cell_tracker/proc/recalculate_type_members(turf/center, type)
	...
	switch(type)
		...
		if(SPATIAL_GRID_CONTENTS_TYPE_RADIO_ATMOS)
			for(var/datum/spatial_grid_cell/cell as anything in new_and_old[1])
				new_members += cell.radio_atmos_contents
			for(var/datum/spatial_grid_cell/cell as anything in new_and_old[2])
				former_members += cell.radio_atmos_contents
		if(SPATIAL_GRID_CONTENTS_TYPE_RADIO_NONATMOS)
			for(var/datum/spatial_grid_cell/cell as anything in new_and_old[1])
				new_members += cell.radio_nonatmos_contents
			for(var/datum/spatial_grid_cell/cell as anything in new_and_old[2])
				former_members += cell.radio_nonatmos_contents
