...
/datum/station_goal/dna_vault/New()
	...
	human_count = rand(QUESTIONABLE_FLOOR(0.75 * SSticker.totalPlayersReady) , SSticker.totalPlayersReady) // 75%+ roundstart population.
	var/non_standard_plants = non_standard_plants_count()
	plant_count = rand(QUESTIONABLE_FLOOR(0.5 * non_standard_plants),QUESTIONABLE_FLOOR(0.7 * non_standard_plants))
