...
/datum/unit_test/translate_speech
	var/mob/host_mob

/datum/unit_test/translate_speech/Run()
	host_mob = allocate(/mob/living/carbon/human/consistent)
	var/surfer_quote = "surfing in the USA"

	var/datum/language/lang_instance = GET_LANGUAGE_DATUM(/datum/language/beachbum)
	host_mob.grant_language(/datum/language/beachbum, FALSE, TRUE) // can speak but can't understand
	host_mob.add_blocked_language(subtypesof(/datum/language) - /datum/language/beachbum, LANGUAGE_STONER)

	TEST_ASSERT_NOTEQUAL(surfer_quote, host_mob.translate_speech(host_mob, lang_instance, surfer_quote), "Language test failed. Mob was supposed to understand: [surfer_quote]")

	host_mob.grant_language(/datum/language/beachbum) // can now understand
	TEST_ASSERT_EQUAL(surfer_quote, host_mob.translate_speech(host_mob, lang_instance, surfer_quote), "Language test failed. Mob was supposed NOT to understand: [surfer_quote]")
...
/datum/unit_test/mute/Run()
	...
	ADD_TRAIT(user, TRAIT_MUTED, TRAIT_SOURCE_UNIT_TESTS)
