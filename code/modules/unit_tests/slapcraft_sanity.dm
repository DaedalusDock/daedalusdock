/datum/unit_test/slapcraft_recipe_sanity/Run()
	...
	for(var/datum/slapcraft_recipe/R as anything in typesof(/datum/slapcraft_recipe))
		...
		if(!step_one.check_types)
			TEST_FAIL("Slapcrafting recipe of type [type] has first step [step_one.type] which doesn't type check. This is incompatible with an optimization cache.")

		// Make sure all steps are unique
		var/list/assoc_check = list()
		for(var/step_type in R.steps)
			if(assoc_check[step_type])
				TEST_FAIL("Slapcrafting recipe of type [type] has duplicate step [step_type]. Steps within a recipe must be unique!")
			...
			switch(step_order)
				if(SLAP_ORDER_STEP_BY_STEP, SLAP_ORDER_FIRST_AND_LAST)
					...
					if(step_count == 1 || step_count == R.steps.len)
						TEST_FAIL("Slapcrafting recipe of type [type] has an optional step [step_type] as first or last step. This is forbidden!")
				if(SLAP_ORDER_FIRST_THEN_FREEFORM)
					TEST_FAIL("Slapcrafting recipe of type [type] has an optional step [step_type] while the order is SLAP_ORDER_FIRST_THEN_FREEFORM. This is forbidden!")
