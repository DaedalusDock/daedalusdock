...
/obj/item/ttsdevice/attack_self(mob/user)
	...
	playsound(src, 'modular_pariah/master_files/sound/items/tts/started_type.ogg', 50, TRUE)
	...
	if(!input)
		...
		playsound(src, 'modular_pariah/master_files/sound/items/tts/stopped_type.ogg', 50, TRUE)
		return
	src.say(input)
	input = null
